...
export abstract class RList<E> extends Source<ReadonlyList<E>> implements ReadonlyList<E> {
  ...
  onEmit(fn: ValueFn<ReadonlyList<E>>): Remover {
    return this.onChange((_) => fn(this))
  }
...
export abstract class MutableList<E> extends RList<E> {
  ...
  static local<E>(eq: Eq<E> = refEquals): MutableList<E> {
    return new LocalMutableList<E>(eq)
  }

  /** Creates a local mutable list. Elements will be compared for equality using [[dataEquals]]. */
  static localData<E extends Data>(): MutableList<E> {
    return this.local<E>(dataEquals)
  }
...
class LocalMutableList<E> extends MutableList<E> {
  ...
  constructor(readonly eq: Eq<E>) {
    super()
  }

  /** Appends `elem` to this list, notifying observers of the change. */
  override append(elem: E) {
...
export abstract class RSet<E> extends Source<ReadonlySet<E>> implements ReadonlySet<E> {
  ...
  forEach(fn: (e: E, edup: E, s: ReadonlySet<E>) => void) {
  ...
  onEmit(fn: ValueFn<ReadonlySet<E>>): Remover {
    return this.onChange((_) => fn(this.data))
  }
...
export abstract class MutableSet<E> extends RSet<E> implements Set<E> {
  ...
  protected abstract override get data(): Set<E>

  /** Creates a local mutable set. */
  static local<E>(): MutableSet<E> {
    return new LocalMutableSet()
  }
  ...
  override forEach(fn: (e: E, edup: E, s: Set<E>) => void) {
    this.data.forEach(fn)
  }
...
export abstract class RMap<K, V> extends Source<ReadonlyMap<K, V>> implements ReadonlyMap<K, V> {
  ...
  static empty<K, V>(): RMap<K, V> {
    ...
    class EmptyMap extends RMap<K, V> {
      ...
      onChange(_: (change: MapChange<K, V>) => any): Remover {
        return NoopRemover
      }
    }
    return new EmptyMap()
  ...
  forEach(fn: (v: V, k: K, m: ReadonlyMap<K, V>) => void) {
    this.data.forEach(fn)
  }
  ...
  get [Symbol.toStringTag](): string {
    return (this.data as Map<K, V>)[Symbol.toStringTag]
  }
  ...
  get keysValue(): Value<Iterable<K>> {
    ...
    const value = Value.deriveValue(
      ...
      (disp) =>
        this.onChange((c) => {
          if (c.type === "deleted")
            disp(keysIable, { [Symbol.iterator]: () => iteratorPlus(this.keys(), c.key) })
          else if (c.prev === undefined)
            disp(
              keysIable,
              filteredIterable(keysIable, (k) => k !== c.key)
            )
        }),
      () => keysIable
    )
...
export abstract class MutableMap<K, V> extends RMap<K, V> implements Map<K, V> {
  ...
  protected abstract override get data(): Map<K, V>

  /** Creates a local mutable map. */
  static local<K, V>(): MutableMap<K, V> {
    return new LocalMutableMap()
  }
  ...
  getMutable(key: K, eq: Eq<V | undefined> = refEquals): Mutable<V | undefined> {
    return this.projectMutable<V | undefined>(
      key,
      (v) => v,
      (_, v) => v,
      eq
  ...
  projectMutable<W>(
    key: K,
    proj: (v: V | undefined) => W,
    inj: (v: V | undefined, w: W) => V | undefined,
    eq: Eq<W | undefined> = refEquals
  ): Mutable<W> {
  ...
  override forEach(fn: (v: V, k: K, m: Map<K, V>) => void) {
    this.data.forEach(fn)
  }
