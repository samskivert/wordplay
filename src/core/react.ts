...
export abstract class Source<T> {
  ...
  fold<Z>(start: Z, fn: (a: Z, v: T) => Z, eq: Eq<Z> = refEquals): Value<Z> {
    ...
    return Value.deriveValue(
      ...
      (disp) =>
        this.onValue((value) => {
          const ovalue = current,
            nvalue = fn(ovalue, value)
          if (!eq(ovalue, nvalue)) {
            current = nvalue
            disp(nvalue, ovalue)
          }
...
export abstract class ReadableSource<T> extends Source<T> implements RProp<T> {
   ...
   * emits a `value`, the returned source will emit `fn(value)`. */
  abstract override map<U>(fn: (v: T) => U): ReadableSource<U>
  ...
  override once(fn: ValueFn<T>): Remover {
    fn(this.current)
    return NoopRemover
  }

  override whenOnce(pred: Pred<T>, fn: ValueFn<T>): Remover {
...
export class Stream<T> extends Source<T> {
  ...
  constructor(protected readonly _onEmit: (fn: ValueFn<T>) => Remover) {
    super()
  }
...
export class Emitter<T> extends Stream<T> {
  ...
  constructor() {
    super((lner) => addListener(this._listeners, lner))
  }
...
export class Subject<T> extends Source<T> {
  ...
  static join<A>(...sources: Source<A>[]): Subject<A[]> {
    return Subject.deriveSubject((dispatch) => {
      const ready: boolean[] = []
      const current: A[] = []
      const removers = sources.map((source, idx) =>
  ...
  constructor(readonly _connect: (lner: ValueFn<T>, wantValue: boolean) => Remover) {
    super()
  }
...
export class Value<T> extends ReadableSource<T> {
  ...
  static from<T>(source: Source<T>, start: T, eq: Eq<T> = refEquals): Value<T> {
    return source.fold(start, (_, n) => n, eq)
  }
   ...
   * state. */
  static deriveValue<T>(
    eq: Eq<T>,
    connect: (dispatch: ChangeFn<T>) => Remover,
    current: () => T
  ): Value<T> {
   ...
   * semantics. */
  static override switch<T>(values: Value<Value<T>>): Value<T> {
    return Value.deriveValue(
      values.current.eq,
      (disp) => {
   ...
   * @return a remover thunk (invoke with no args to unregister `fn`). */
  onChange(fn: ChangeFn<T>): Remover {
    return this._onChange(fn)
  }
  ...
  map<U>(fn: (v: T) => U, eq: Eq<U> = refEquals): Value<U> {
    ...
    return Value.deriveValue(
      ...
      (disp) => {
        connected = true
        latest = fn(_current())
        const unlisten = _onChange((value, _) => {
...
export class Mutable<T> extends Value<T> implements Prop<T> {
  ...
  static local<T>(start: T, eq: Eq<T> = refEquals): Mutable<T> {
    ...
    return new Mutable(
      eq,
      (lner) => addListener(listeners, lner),
      () => current,
      (newValue) => {
  ...
  bimap<U>(project: (t: T) => U, inject: (t: T, u: U) => T): Mutable<U> {
    ...
    return new Mutable(
      eqU,
      (disp) =>
        _onChange((value: T, ovalue: T) => {
          let current = project(value),
            previous = project(ovalue)
          if (!eqU(current, previous)) disp(current, previous)
        }),
      () => project(_current()),
      (u: U) => _update(inject(_current(), u))
    )
  }
}
...
export abstract class Buffer<T> extends ReadableSource<T> implements Prop<T> {
  ...
  private static readonly defaultUpdater: (o: any, n: any) => any = (_, n) => n
  ...
  abstract override get current(): T
...
class ValueBuffer<T> extends Buffer<T> {
  constructor(
    public current: T,
    private readonly updater: (o: T, n: T) => T
  ) {
    super()
  }
