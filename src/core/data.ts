...
export function dataCopy<T extends Data>(value: T): T {
  ...
  if (Array.isArray(value)) {
    ...
    const setv = value as Set<Data>,
      setc = new Set<Data>()
    setv.forEach((elem) => setc.add(dataCopy(elem)))
    return setc as T
  } else if (isMap(value)) {
    const mapv = value as Map<DataMapKey, Data>,
      mapc = new Map<DataMapKey, Data>()
    ...
    const recv = value as Record,
      recc = Object.create(Object.getPrototypeOf(value))
...
export function dataEquals(a: Data | undefined, b: Data | undefined): boolean {
  ...
  for (const key in a)
    if (a.hasOwnProperty(key) && !dataEquals((a as any)[key], (b as any)[key])) return false
